                    ...
                    ?>
                        <div class="col-sm-6" style="width: 40% ;">
                            ...
                            <form action="" method="POST">
                                ...
                                <div class="form-group">
                                    <label>Họ tên:</label>
                                    <input type="text" class="form-control" placeholder="Nhập họ và tên đầy đủ" <?= $value_hoten ?> disabled readonly>
                                </div>
                                <div class="form-group">
                                    <label>Email:</label>
                            ...
                            <table class="table table-condensed table-hover">
                                ...
                                <?php
                                    ...
                                    if (!empty($_GET['idmess']) && !empty($_GET['id']) && !empty($_GET['noidung'])) {
                                        $value_mess = 'value="' . htmlspecialchars($_GET['noidung']) . '"';
                                    }
                                ?>
                                    <form action="" method="POST">
                                    ...
                                    if (isset($_POST['click_submit_mess'])) {
                                        if (!empty($_POST['input_mess'])) {
                                            ...
                                            if (empty($_GET['idmess'])) {
                                                ...
                                                $stmt = $conn->prepare('UPDATE messenger SET noidung=? WHERE idmess=? AND idgui=?');
                                                $stmt->bind_param('sii', $_POST['input_mess'], $_GET['idmess'], $_SESSION['id']);
                                    ...
                                    <thead>
                                        <tr>
                                            <th>Username: <?= htmlspecialchars($_GET["username"]) ?></th>
                                            <th></th>
                                            <th></th>
                                        </tr>
                                    ...
                                    ?>
                                        <tr>
                                            ...
                                            ?>
                                                ...
                                                <td><a href="?idmess=<?= $mess2['idmess'] ?>&id=<?= $_GET['id'] ?>&username=<?= htmlspecialchars($_GET["username"]) ?>&noidung=<?= htmlspecialchars($mess2['noidung']) ?>"><img src="./image/edit.png" width="70%"></a></td>
                                                <td><a href="delete.php?idmess=<?= $mess2['idmess'] ?>&idttcn=<?= $_GET['id'] ?>&username=<?= htmlspecialchars($_GET["username"]) ?>&token=<?=$token?>"><img src="./image/delete.png" width="70%"></a></td>
                                            <?php
                                            } else {
                                            ?>
                                                <td style="width: 90%;"><?= $mess2['time'] . ' [' . htmlspecialchars($_GET["username"]) . '] ' . htmlspecialchars($mess2['noidung']) ?></td>
                                                <td></td>
                                                <td></td>
                                            <?php
